...
import { IOrdenesVenta } from '../lista-pedidos-pendientes-sap/lista-pedidos-pendientes-sap.component';
...
@Component({
  selector: 'app-pedidos-enviados-sap',
  templateUrl: './pedidos-enviados-sap.component.html',
  styleUrls: ['./pedidos-enviados-sap.component.scss']
})
export class PedidosEnviadosSAPComponent implements OnInit, AfterViewInit {
  ...
  obtenerOrdenesVenta() {
    ...
    this.ordenesVentaService.ObtenerOrdenesVentaEnviadasSAPGTS().subscribe(
      (data) => {
    ...
    this.ordenesVentaService.generarPedidosEnSAP(ids).subscribe(
      (response) => {
        ...
        if (response.errors && response.errors.length > 0) {
          const errorMessages = response.errors
            .map((error: any) => `Pedido ID: ${error.idPedidoCaja}, Error: ${error.error || 'Error desconocido'}`)
            .join('<br>');
      ...
      (error) => {
        ...
        if (error.error && error.error.details && error.error.details.length > 0) {
          errorMessage = error.error.details.map((detail: any) => `Pedido ID: ${detail.idPedidoCaja}, Error: ${detail.error}`).join('<br>');
        } else if (error.status === 500) {
    ...
    this.ordenesVentaService.obtenerDetalleOrdenVenta(element.Id_pedido_Caja).subscribe(
      (data) => {
        const dialogRef = this.dialog.open(DetallePedidosComponent, {
          ...
          data: { ordenVenta: element, detalles: data.detalles }
        });
  ...
  descargarTemplatePedidos() {
    ...
    link.href = 'assets/Templates/TemplateCargapedidos.xlsx';
    link.download = 'TemplateCargapedidos.xlsx';
    link.click();
  }
